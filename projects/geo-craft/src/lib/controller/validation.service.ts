...
export class ValidationService {
  ...
  async identifyStep(userObject: any, toolSelected: string): Promise<any> {
    try {
      ...
      const stepIdentification = await this.hintGenerationService.identifyStep(
        userObject,
        this.getPendingSteps(toolSelected),
        this.config.labelSensitive
      );
      console.log('Step identified by assistant:', stepIdentification);

      return await this.hintGenerationService.parseResponse(stepIdentification);
  ...
  async generateHints(config: any) {
    try {
      ...
      const hint = await this.hintGenerationService.generateHints(
        config,
        this.completedStepMap
      );
      console.log('hint generated by assistant:', hint);

      const result = await this.hintGenerationService.parseResponse(hint);
  ...
  async startValidation() {
    ...
    if (!stepId) {
      this.configForHint['userObject'] = null;
      ...
      this.configForHint['userObject'] = lastElement;
      this.configForHint['step'] = step;
      ...
      if (!depsMet && !this.isStepCompleted(stepId)) {
        ...
        if (result.data) this.configForHint['data'] = result.data;
