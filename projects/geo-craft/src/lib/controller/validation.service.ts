...
import { type } from '../config/animation.json';
...
export class ValidationService {
  ...
  // @ViewChild(GeoCraftViewComponent) geoCraftView:
  ...
  constructor(
    ...
    private fabricRenderer: FabricRendererService
  ) {}
  ...
  async startValidation() {
    ...
    console.log(lastElement, lastIndex, tool);
    ...
    // lastElement.updateStroke(this.fabricRenderer, this);

    // const matchingDrawable = elements.find((d) => {
    //   const pointOrSegment = d.getPoint?.() || d.segment || d.line;
    //   console.log(pointOrSegment,'matchingDrawable');
    //   return (
    //     pointOrSegment?.label === lastElement.label || pointOrSegment?.id === lastElement.id
    //   );
    // });

    // if (matchingDrawable?.updateStroke) {
    //   matchingDrawable.updateStroke('red');
    // }

    // lastElement.updateStroke('red');
    ...
    if (!stepId) {
      ...
      if (!depsMet && !this.isStepCompleted(stepId)) {
        ...
        if (result.matched) {
          ...
          if (result.reason) {
            this.viewState.validateLastElement(type.green);
            this.viewState.emitmessage(
              this.isComplete()
                ? 'Great job! You’ve completed this question successfully!'
                : result.reason
            );
            return;
          }
        ...
        if (result.data)
          this.configForHint['userStep']['data'] = { ...result.data };
    ...
    this.viewState.validateLastElement(type.red);
