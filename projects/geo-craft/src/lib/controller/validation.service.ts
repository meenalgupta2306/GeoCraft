...
export class ValidationService {
  ...
  async identifyStep(userObject: any): Promise<any> {
    try {
  ...
  async generateHints(config: any): Promise<any> {
    try {
      ...
      return await this.hintGenerationService.parseResponse(hint);;
      // Optionally emit or handle the result
    } catch (err) {
      console.error('Error during step identification:', err);
      return '';
  ...
  async startValidation() {
    ...
    const stepToMatch = await this.identifyStep(lastElement);
    ...
    if (!stepId) {
      ...
      const step = pendingSteps[stepId -1];
      console.log(stepToMatch);
      this.configForHint["userStep"] = step;
      ...
      if (depsMet) {
        ...
        this.configForHint['dependentStep'] = pendingSteps[unmetDependencyStep];
      }
    ...
    const result = await this.generateHints(this.configForHint);
